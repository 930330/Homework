...
## 解題說明
本題要求使用循環鏈結串列實作多項式類別，並支援輸入、輸出、加減乘及求值

## 解題策略
類別設計

內部私有類別 Term 儲存每一項的 coef（係數）與 exp（指數）。

Polynomial 類別包含動態陣列 termArray 來管理所有非零項。

1.建構子與解構子管理動態記憶體。

多項式加法（Add）合併同類項，時間複雜度 O(m+n)。

多項式乘法（Mult）使用分配律展開並合併同類項，時間複雜度 O(m×n)。

多項式求值（Eval）逐項使用 pow() 計算。
